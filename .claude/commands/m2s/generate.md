...
allowed-tools: Bash(mcp2skill:*), Read, Write, AskUserQuestion
---
...
Get tool information: !`mcp2skill tools $1`
...
## Step 3: For Complex skills (>10 tools) - Suggest grouping
...
## Step 4: Generate Python scripts

Run: !`mcp2skill generate $1`

This creates:
- `~/.claude/skills/mcp-$1/scripts/` directory
- `mcp_client.py` - Shared REST client
- One Python script per tool (with argparse)

Verify scripts were created: !`ls ~/.claude/skills/mcp-$1/scripts/`

## Step 5: Create SKILL.md with best practices

**Read the existing SKILL.md** (if generated by mcp2skill): !`cat ~/.claude/skills/mcp-$1/SKILL.md`
...
Show user the first 50 lines of SKILL.md: !`head -50 ~/.claude/skills/mcp-$1/SKILL.md`
...
```
✓ Generated skill: ~/.claude/skills/mcp-$1/
