...
allowed-tools: Bash(mcp2skill:*), Bash(ls:*), Bash(cat:*), Bash(head:*), Read, Write, AskUserQuestion
---

Generate a Claude Code skill for MCP server "$1" with LLM-enhanced documentation following best practices.

**Reference:** @skill-authoring-guide.md for all best practices to follow.

## Step 1: Analyze the MCP server

Use Bash to get tool information: `mcp2skill tools $1`
...
1. **User skills** (`~/.claude/skills/`)
   - Description: "Available globally across all projects. Use for tools you'll use everywhere (browser automation, file operations, etc.)"

2. **Project skills** (`./.claude/skills/`)
   - Description: "Available only in this project. Use for project-specific integrations or when testing skills before promoting to user-level."

**Store the answer** in a variable for use in subsequent steps:
- If user chooses "User skills": `SKILL_PATH=~/.claude/skills`
- If user chooses "Project skills": `SKILL_PATH=./.claude/skills`
...
## Step 5: Generate Python scripts

Use Bash to run: `mcp2skill generate $1 --output $SKILL_PATH`

Where `$SKILL_PATH` is the location chosen in Step 3.

This creates:
- `$SKILL_PATH/mcp-$1/scripts/` directory
- `mcp_client.py` - Shared REST client
- One Python script per tool (with argparse)

Use Glob to verify scripts were created: `$SKILL_PATH/mcp-$1/scripts/*.py`

## Step 6: Create SKILL.md with best practices

**Read the existing SKILL.md** (if generated by mcp2skill) using Read tool: `$SKILL_PATH/mcp-$1/SKILL.md`

**Now enhance it** following @skill-authoring-guide.md:
...
- **tool_name** - Description
  ...
  - Optional: [--param3]
  - Example: `python scripts/tool_name.py --param1 value --param2 value`
...
Steps:
```bash
python scripts/fill.py --uid email_input_abc123 --value user@example.com
python scripts/fill.py --uid pass_input_def456 --value secretpass
python scripts/click.py --uid submit_btn_ghi789
```
...
- [ ] Open target page: `python scripts/new_page.py --url URL`
- [ ] Get page structure: `python scripts/take_snapshot.py`
- [ ] Identify form element UIDs from snapshot
- [ ] Fill form fields with `fill.py` (repeat for each field)
- [ ] Submit form: `python scripts/click.py --uid submit_btn_uid`
- [ ] Verify success: `python scripts/take_screenshot.py`
```

## Step 7: Validate against best practices

Check the generated SKILL.md against @skill-authoring-guide.md:
...
**Next steps:**
1. Claude Code auto-discovers skills in `~/.claude/skills/` (user) and `./.claude/skills/` (project)
2. Test the skill: Try using the tools via Python scripts
3. Iterate: Use `/m2s:update $1` to improve the skill
4. Share: Skills can be distributed via git
